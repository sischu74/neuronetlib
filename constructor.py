...
class node():
    def __init__(self, w, b=0, child_nodes=[], parent_nodes=[], value=0, derivative=0):
        ...
        self.b = b # this node's offset parameter
...
class ffnn():
    ...
    def predict(self, x, y):
        ...
        for sample in range(x.shape[0]):
            ...
            # calculate all node values as the dot product of its w vector and the parent nodes' values plus an offset 'b'
            for i in range(1,len(layers)): # cycle through layers
                for neuron in layers[i].nodes: # cycle through nodes in layer
                    neuron.value = np.dot(neuron.w, layers[i-1].value_list()) + neuron.b
            ...
            if self.task == 'classification':
                for neuron in range(len(layers[-1].nodes)): # cycle through output nodes
                    denominator = 0
                    for others in range(len(layers[-1].nodes)): # cycle through other nodes in the layer
                        denominator += np.exp(layers[-1].nodes[others].value)
                    neuron.value = np.exp(neuron.value)/denominator
    ...
    def predictions(self, x, y):
        ...
        print('Test Error (per sample):', errors[0]/x.shape[0])
    ...
    def construct(self, x):
        ...
        for i in range(1, len(self.h_layers)+1):
            for neuron in layers[i].nodes:
                neuron.w.extend(np.random.uniform(-5,5,len(layers[i-1].nodes)))
    ...
    def train(self, x, y):
        ...
        while self.early_stopping(x,y,iteration) == False: # check for convergence and early stopping criteria
            ...
            for sample in range(mini_batch.shape[0]):
                ...
                error, prediction = self.predict(np.array(mini_batch[sample,0]).reshape(1,1),np.array(mini_batch[sample,1]).reshape(1,1))

                for neuron in layers[-1].nodes: # calculate partial derivatives for nodes in the last layer
                    neuron.derivative = (-2/mini_batch.shape[0])*(mini_batch[sample,1]-neuron.value)
        ...
        errors, preds = self.predict(x,y) # predict with the adjusted parameters
        print('Iterations until stopped:', iteration)
        print('Training error (per sample):', errors[0]/x.shape[0])
    ...
    def early_stopping(self, x, y, iteration):
        ...
        if iteration == 0: # if it's the first iteration, skip everything here right away
            ...
            if np.sqrt((error[0]-last_error)**2) < 0.000001: # if error barely changes with an iteration, declare convergence
                if iteration > 1000:
